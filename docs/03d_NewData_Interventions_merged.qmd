...
```{r}
df_raw <- googlesheets4::read_sheet(ss = "https://docs.google.com/spreadsheets/d/1wiiuNVE2Wt-ZWyZRWDoqyxvg-y-hWpNdDWwdZjYCvM4/edit#gid=1180615821",
                                    sheet = "03b_interventions_newdata_dbpedia_noid")

# Create a usable df so we don't have to pull from the googlesheet every time we overwrite something
df <- df_raw

df_merged <- read.csv(paste0(here::here(),"/data/02c_interventions_newdata_merged.csv"))
```
...
## Wikidata
For every wikidata ID i in ids, return the value for each desired wikidata quality p
...
### Clean data
We now have a df of all covariates that we can extract from wikidata
...
Merge into the df from 03a. This is the important step!
```{r}
